...
ti.root.pointer(ti.ij, ((nx + 1) * ny // ptr_size, (nx + 1) * ny // ptr_size)).dense(ti.ij, (ptr_size, ptr_size)).place(
    Au, Mu)
...
ti.root.pointer(ti.ij, (nx * (ny + 1) // ptr_size, nx * (ny + 1) // ptr_size)).dense(ti.ij, (ptr_size, ptr_size)).place(
    Av, Mv)
    ...
    for _ in range(1):
        for steps in range(100 * n):
            ...
            for i in range(n):
                residual += r[i] * r[i]
            if output:
                print("        >> Iteration ", steps, ", initial residual = ", residual_init, ", current residual = ",
                      residual)
...
if __name__ == "__main__":
    ...
    for time_step in range(500000):
        ...
        for subtime_step in range(500):
            ...
            print(
                f'    >> [ time = {time_step * dt: .4f} ] It took {time.time() - start: .2f} sec to solve the momentum equation.')
            ...
            post_process_field()
            print(
                f'    >> [ time = {time_step * dt: .4f} ] The current pcor_max is {pcor_max: .3e} and the current p-correction compared with pcor_max is {(pcor_current / pcor_max): .3e}')
        ...
        img = np.concatenate(
            (pcor_disp.to_numpy(), udiv_disp.to_numpy(), p_disp.to_numpy(), u_disp.to_numpy(), v_disp.to_numpy()),
            axis=1)
        gui.set_image(img)
